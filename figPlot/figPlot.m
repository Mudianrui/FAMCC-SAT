...
case 1
    for i=1:size(x.signals.values,2)
        plot(x.time,x.signals.values(:,i),'LineWidth',2);hold on;
    end
case 2
    % 仅设定了线条宽度
    if isa(varargin{1},'double')
        ...
        for i=1:size(x.signals.values,2)
            plot(x.time,x.signals.values(:,i),'LineWidth',lineWidth);hold on;
        end
    % 仅设定了每个线条的显示方式
...
function plotWithType(x,lineWidth,lineType)
    if size(lineType,1)>size(x.signals.values,2)
        error('信号数量比预期的少');
    end
    if size(lineType,2)==1
        for i=1:size(lineType,1)
            plot(x.time,x.signals.values(:,i),lineType{i},'LineWidth',lineWidth);hold on;
        end
    elseif size(lineType,2)==2
        for i=1:size(lineType,1)
            plot(x.time,x.signals.values(:,lineType{i,1}),lineType{i,2},'LineWidth',lineWidth);hold on;
        end
    end
end
